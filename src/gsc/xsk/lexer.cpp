...
xsk::gsc::parser::symbol_type GSClex(xsk::gsc::lexer& lexer)
{
    return lexer.lex();
}
...
lexer::lexer(const context* const ctx, const std::string& name, const char* data, size_t size) : ctx_(ctx),
    loc_(location(&name)), locs_(std::stack<location>()), readers_(std::stack<reader>()), header_top_(0), indev_(false), clean_(true)
{
...
void lexer::push_header(const std::string& file)
{
    try
    {
        ...
        auto data = ctx_->file_data(file + ".gsh");
        ...
        else
        {
            ...
            if (buffer_.length < 17)
            {
                ...
                if (itr != keyword_map.end())
                {
                    ...
                    {
                        if (ctx_->props() & props::is_defined_is_true)
                            return parser::symbol_type(itr->second, loc_);
                    }
                    else
                    {
