...
disassembler::disassembler(context::ref ctx) : ctx_(ctx) {}
...
auto disassembler::output_data() -> std::vector<std::uint8_t>
{
    ...
    output_->write_string("// IW8 GSC ASSEMBLY\n");
...
{
    filename_ = file;
    script_ = std::make_unique<utils::reader>(script, ctx_->endian() == endian::big);
    stack_ = std::make_unique<utils::reader>(stack, ctx_->endian() == endian::big);
    functions_.clear();
    ...
    while (stack_->is_avail() && script_->is_avail())
    {
        ...
        func->id = (ctx_->props() & props::token_size) ? stack_->read<std::uint32_t>() : stack_->read<std::uint16_t>();
        func->name = func->id == 0 ? decrypt_string(stack_->read_c_string()) : ctx_->token_name(func->id);
...
void disassembler::dissasemble_function(const function::ptr& func)
{
    ...
    while (size > 0)
    {
        ...
        inst->opcode = ctx_->opcode_enum(script_->read<std::uint8_t>());
...
//      case opcode::OP_manualAndAssignmentBreakpoint:
        ...
        case opcode::OP_GetIString:
            script_->seek((ctx_->props() & props::string_size) ? 4 : 2);
            inst->data.push_back(utils::string::to_literal(decrypt_string(stack_->read_c_string())));
            break;
        case opcode::OP_GetAnimation:
            script_->seek((ctx_->props() & props::string_size) ? 8 : 4);
...
void disassembler::disassemble_builtin_call(const instruction::ptr& inst, bool method, bool args)
{
    ...
    const auto name = method ? ctx_->meth_name(id) : ctx_->func_name(id);
...
void disassembler::disassemble_far_call(const instruction::ptr& inst, bool thread)
{
    ...
    const auto file_id = (ctx_->props() & props::token_size) ? stack_->read<std::uint32_t>() : stack_->read<std::uint16_t>();
    const auto file_name = file_id == 0 ? decrypt_string(stack_->read_c_string()) : ctx_->token_name(file_id);
    const auto func_id = (ctx_->props() & props::token_size) ? stack_->read<std::uint32_t>() : stack_->read<std::uint16_t>();
    const auto func_name = func_id == 0 ? decrypt_string(stack_->read_c_string()) : ctx_->token_name(func_id);
...
void disassembler::disassemble_end_switch(const instruction::ptr& inst)
{
    ...
    if (count)
    {
        for (auto i = count; i > 0; i--)
        {
            ...
            if (value < 0x100000 && value > 0) // iw5 iw6 iw7, s1, h1, h2: 0x40000
            {
...
{
    const auto id = (ctx_->props() & props::token_size) ? script_->read<std::uint32_t>() : script_->read<std::uint16_t>();
    std::string name;

    if (id > ctx_->str_count())
    {
        auto temp = (ctx_->props() & props::token_size) ? stack_->read<std::uint32_t>() : stack_->read<std::uint16_t>();
...
auto disassembler::disassemble_offset() -> std::int32_t
{
    ...
    {
        bytes[0] = script_->read<std::uint8_t>();
        bytes[1] = script_->read<std::uint8_t>();
        bytes[2] = script_->read<std::uint8_t>();
    }
    else
    {
        bytes[2] = script_->read<std::uint8_t>();
...
auto disassembler::decrypt_string(const std::string& str) -> std::string
{
    if (str.size() > 0 && ((static_cast<std::uint8_t>(str[0]) & 0xC0) == 0x80))
    {
        ...
        {
            data += utils::string::va("%02X", static_cast<std::uint8_t>(str[i]));
        }
...
{
    output_->write_string(utils::string::va("\t\t%s", ctx_->opcode_name(inst->opcode).data()));
