...
compiler::compiler(context::ref ctx) : ctx_(ctx) {}
...
auto compiler::parse_buffer(const std::string& file, const char* data, size_t size) -> ast::program::ptr
{
    ...
    lexer lexer(ctx_, file, data, size);
...
void compiler::emit_expr_and(const ast::expr_and::ptr& expr, const block::ptr& blk)
{
    ...
    if (expr->rvalue == ast::kind::expr_not && (ctx_->props() & props::bool_not_after_and))
    {
...
{
    if (ctx_->props() & props::formal_params)
    {
